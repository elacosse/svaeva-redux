...
.PHONY: test
test:
	PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=svaeva_redux tests/
...
check-codestyle:
	poetry run darglint --verbosity 2 svaeva_redux tests
...
check-safety:
	poetry check
	poetry run bandit -ll --recursive svaeva_redux tests
